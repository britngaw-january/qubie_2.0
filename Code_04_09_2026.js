...
// - Inbound C25 / Outbound C28 only evaluated when account is resolved / settlement payment completed
// - For C25 / C28, timeline language like "up to 45 days" satisfies the criterion
...
// - Inbound C11 / Outbound C14 contact information:
//   * If DSC/third party or STATUS='canceled', email-only confirmation is sufficient
...
function buildEvaluationPrompt_(rowCtx) {
  ...
  const bilingualInstructions = rowCtx.language === 'Spanish'
    ? [
        ...
        'Recognize Spanish equivalents of required disclosures, authentication, settlement language, recording disclosure, payment terms, dispute language, badge number, reporting timeline, transfer language, and debt descriptions.',
  ...
  const rules = [
    ...
    '- Reporting timeline criteria apply ONLY when the account is resolved or settlement payment is completed.',
    '- Reporting timeline is satisfied if the agent says it will take up to a number of days, such as "up to 45 days".',
    ...
    '  * For contact information criteria, if this is DSC/third-party or STATUS = canceled, an email alone is sufficient and full borrower contact confirmation is not required.',
...
function applyHardRuleOverrides_(evaluations, rowCtx) {
  ...
  if (rowCtx.direction === 'INBOUND' && byId['25']) {
    if (!rowCtx.flags.resolutionCompleted) {
      byId['25'].verdict = 'N/A';
      byId['25'].evidence = 'Not evaluated because the account was not resolved or settlement payment was not completed.';
    } else if (rowCtx.flags.hasReportingTimelineDisclosure) {
      byId['25'].verdict = 'Yes';
      byId['25'].evidence = 'Agent provided a reporting timeline such as up to a number of days.';
    } else {
      byId['25'].verdict = 'No';
      byId['25'].evidence = 'Account was resolved, but reporting timeline language like "up to X days" was not clearly stated.';
    }
  }

  if (rowCtx.direction === 'OUTBOUND' && byId['28']) {
    if (!rowCtx.flags.resolutionCompleted) {
      byId['28'].verdict = 'N/A';
      byId['28'].evidence = 'Not evaluated because the account was not resolved or settlement payment was not completed.';
    } else if (rowCtx.flags.hasReportingTimelineDisclosure) {
      byId['28'].verdict = 'Yes';
      byId['28'].evidence = 'Agent provided a reporting timeline such as up to a number of days.';
    } else {
      byId['28'].verdict = 'No';
      byId['28'].evidence = 'Account was resolved, but reporting timeline language like "up to X days" was not clearly stated.';
    }
  }

  // C2 debt-details leniency for canceled status
  ...
  // Contact info: for DSC/third party or canceled status, email alone is sufficient
  ...
  if (byId[contactId]) {
    if (rowCtx.flags.relaxedContactInfoScenario) {
      if (rowCtx.flags.hasEmailContactInfo) {
        ...
        byId[contactId].evidence = 'Email contact information was captured, which is sufficient for canceled/DSC/third-party scenarios.';
      } else if (byId[contactId].verdict === 'No') {
        byId[contactId].evidence = 'Email contact information was not clearly captured, which is required in this canceled/DSC/third-party scenario.';
      }
    }
  }

  // Recap leniency for canceled status
...
function getKeepCriteriaForCutoff_(direction) {
  if (direction === 'INBOUND') {
    return [1, 2, 3, 4, 7, 8, 9, 11, 12, 13, 16, 21, 22, 25, 26, 27, 28, 29];
  }
  return [1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 12, 14, 15, 18, 22, 27, 28, 29, 30, 31];
}
...
function deriveCallFlags_(transcript, callCode, companyName, isOosRaw, status) {
  ...
  const resolvedPayment = patterns.resolutionCompleted.test(t);
  ...
  const hasReportingTimelineDisclosure = detectReportingTimelineDisclosure_(t);
  ...
  const relaxedContactInfoScenario = normalizedStatus === 'canceled' || unauthorizedThirdParty || dscOrThirdParty;
  ...
  return {
    ...
    hasReportingTimelineDisclosure: hasReportingTimelineDisclosure,
...
function getCriteriaForRow_(rowCtx) {
    ...
    return [
      ...
      crit_(11,
        flags.relaxedContactInfoScenario
          ? 'Agent confirmed or captured at least an email address. For DSC/third-party or canceled status, full borrower contact confirmation is not required.'
      ...
      crit_(25, 'Agent provided January timetable for reporting a resolved account to the creditor.', flags.resolutionCompleted, 'Not evaluated because the account was not resolved or settlement payment was not completed.'),
      crit_(26,
        isCanceled
  ...
  return [
    ...
    crit_(28, 'Agent provided January timetable for reporting a resolved account to the creditor.', flags.resolutionCompleted, 'Not evaluated because the account was not resolved or settlement payment was not completed.'),
...
function detectReportingTimelineDisclosure_(transcript) {
  const t = String(transcript || '');
  return /(up to\s*\d{1,3}\s*days|within\s*\d{1,3}\s*days|take[s]?\s*up to\s*\d{1,3}\s*days|up to\s*\d{1,3}\s*business days|hasta\s*\d{1,3}\s*días|dentro de\s*\d{1,3}\s*días|puede tardar hasta\s*\d{1,3}\s*días|hasta\s*\d{1,3}\s*días hábiles)/i.test(t);
}
