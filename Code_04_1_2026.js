...
function buildEvaluationPrompt_(rowCtx) {
  ...
  const bilingualInstructions = rowCtx.language === 'Spanish'
    ? [
        ...
        'Recognize Spanish equivalents of required disclosures, authentication, settlement language, recording disclosure, payment terms, dispute language, badge number, reporting timeline, and transfer language.',
...
function getKeepCriteriaForCutoff_(direction) {
  if (direction === 'INBOUND') {
    return [1, 2, 3, 4, 7, 8, 9, 11, 12, 13, 16, 21, 22, 25, 27, 28, 29];
  }
  return [1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 12, 14, 15, 18, 22, 28, 29, 30, 31];
}
...
function getCriteriaForRow_(rowCtx) {
  ...
  if (rowCtx.direction === 'INBOUND') {
    return [
      ...
      crit_(9, 'Agent provided the OOS disclosure.', String(rowCtx.isOos) === 'true' || rowCtx.isOos === true, 'Not evaluated because IS_OOS is not OOS.'),
  ...
  return [
    ...
    crit_(8, 'Agent provided the OOS disclosure.', String(rowCtx.isOos) === 'true' || rowCtx.isOos === true, 'Not evaluated because IS_OOS is not OOS.'),
    ...
    crit_(14, 'Agent confirmed or updated borrower contact info (phone, email, address).', !flags.unauthorizedThirdParty, 'Not evaluated because this was an unauthorized third-party call.'),
